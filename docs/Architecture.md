Here you find the architecture documentation of AMIRIS.
This documentation addresses AMIRIS version 5.0.0.

This section is still under construction.
Once complete, it and shall follow the [Arc42](https://arc42.org/) template.

## [Introduction & Goals](./Architecture/Introduction.md)

## [Constraints](./Architecture/Constraints.md)

## [Context & Scope](./Architecture/Context-And-Scope.md)

## [Solution Strategy](./Architecture/Solution-Strategy.md)

<!--
## [Building Block View](./Architecture/Building-Block-View.md)

## [Runtime View](./Architecture/Runtime-View.md)

## [Deployment View](./Architecture/Deployment-View.md)

## [Crosscutting Concepts](./Architecture/Crosscutting-Concepts.md)

## [Architectural Decisions](./Architecture/Architectural-Decisions.md)

## [Quality Requirements](./Architecture/Quality-Requirements.md)

## [Risk & Technical Debt](./Architecture/Risks-And-Technical-Debt.md)
-->

## [Glossary](./Glossary.md)